...
=============================================================================
 Author	: Muhammad Tauqir Hassnain 
...
=============================================================================
 Note : This script supports only SQL Server 2005 or later.
...
*************************  SET PARAMETER *************************
The script has default parameters set, you can change these (optional)
...
WHILE @@fetch_status = 0
 ...
 INSERT INTO ##tmpFragmentedIndexes (
  ...
  SELECT
     ...
     pst.avg_fragmentation_in_percent as [AvgFragmentationPercentage],pst.page_count,
...
BEGIN 
 SELECT DISTINCT dbName ,
            ...
            schemaName ,
             indexName ,            
            AvgFragmentationPercentage,page_count,
            reorganizationOrRebuildCommand
 FROM    ##tmpFragmentedIndexes Where indexName not like '%PK_%' --and dbName = 'SQLPlanner'-- Where page_count > 50
...
BEGIN 
 ...
 FOR
    SELECT distinct (reorganizationOrRebuildCommand)
...
DECLARE @subject NVARCHAR(255) = 'Fragmented Indexes Report';
...
SELECT @body = (
    SELECT '
        <html>
            ...
            <body>
				<p style="font-size: 11px;">The following indexes have been reorganized/rebuilt according to the scheduled design as of "' + CONVERT(NVARCHAR(20), @currentDateTime, 100) + '"</p>
                <table>
                    <tr>
                        <th>Database Name</th>
                        <th>Table Name</th>
                        <th>Index Name</th>
                        <th>Avg Fragmentation Percentage</th>
                        <th>Page Count</th>
                        <th>Reorganization/Rebuild Command</th>
                    </tr>' +
                    (
                        SELECT
                            '<tr>' +
                            ...
                            '<td>' + tableName + '</td>' +
                            '<td>' + indexName + '</td>' +
                            '<td>' + CONVERT(NVARCHAR(50), AvgFragmentationPercentage) + '</td>' +
                            '<td>' + CONVERT(NVARCHAR(50), page_count) + '</td>' +
                            '<td>' + reorganizationOrRebuildCommand + '</td>' +
                            '</tr>'
...
EXEC msdb.dbo.sp_send_dbmail
    @profile_name = 'DBA', -- Specify the Database Mail profile name
    @recipients = 'test@test.com', -- Specify the recipient email address
    @subject = @subject,
    @body = @body,
    @body_format = 'HTML';
